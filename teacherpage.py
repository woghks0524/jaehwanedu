...
def step1():
    ...
    st.info("선생님이 만든 평가를 찾기 위한 평가 코드를 만들어주세요."
            "학생 페이지에서 선생님이 만든 평가를 불러오는 데 사용됩니다.")
...
def step2():
    ...
    st.success("서술형 평가에 활용할 자료를 입력해 주세요."
               "교과서 pdf, 수업 자료 pdf 등을 입력할 수 있습니다."
               "입력한 자료를 근거로 모범 답안이나 피드백을 생성할 수 있습니다.")
...
# vs_FtRt7SEalipabRPrOk0usxl8
# 등록된 자료 확인
        filelist = st.checkbox(
            "등록된 자료 목록 (확인 후에는 체크 박스를 해제 해주세요.)")
        
        if filelist:
            thread_message = client.beta.threads.messages.create(
                thread_id=st.session_state['usingthread'],
                role="user",
                content='vs_FtRt7SEalipabRPrOk0usxl8을 file search합니다.'
                        'vs_FtRt7SEalipabRPrOk0usxl8에 있는 파일 목록을 오름차순으로 정리합니다.'
                        '파일 이름은 파일 확장자 부분을 제거한 다음 한 줄에 하나씩 줄 바꿈해서 제시합니다.'
                        '다른 문장은 넣지 말고 파일 이름만 제시합니다.'
                        '5번 이상 반복적으로 file search한 다음 제시합니다.')
...
def step3():
    ...
    st.error(
    "서술형 평가 문항과 모범답안을 입력하세요. 최대 3개 문항까지 가능합니다."
    "모범답안의 경우 선택사항으로 모범답안을 입력하지 않으면 자동으로 모범답안을 생성합니다."
    "평가 주의사항을 입력하면 이를 근거로 채점 및 피드백을 생성할 수 있습니다.")
...
def step4():
    ...
    st.warning("선생님이 설정한 평가를 확인하고 저장합니다." 
               "수정이 필요하다면 이전 단계로 가서 다시 입력한 다음 저장합니다.")
        ...
        if testcheck:
            ...
            client.beta.threads.messages.create(
                ...
                content='현재 업로드된 서술형 평가 문항과 모범답안, 평가 주의사항을 모두 보여주세요.'
                        ...
                        '표 형태로 정리하여 보여주고 이외에 다른 문장은 넣지 않습니다.')
