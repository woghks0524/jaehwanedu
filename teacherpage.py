...
# 새로운 벡터 스토어를 기존 어시스턴트에 업데이트
            client.beta.assistants.update(
                ...
                tool_resources={"file_search": {"vector_store_ids": [st.session_state['vectorstoreid']]}})
    ...
    with st.container(border=True):
        ...
        if testcheck:
            ...
            client.beta.threads.messages.create(
                thread_id=st.session_state['usingthread'],
                role="user",
                content='현재 업로드된 서술형 평가 문항, 모범답안, 평가 주의사항을 모두 보여주세요. user(teacher)가 입력한 평가 문항을 있는 그대로 보여주세요. 만약 user(teacher)가 입력한 모범답안이 있으면 user(teacher)가 입력한 것을 있는 그대로 보여줍니다. 만약 user(teacher)가 입력한 모범답안이 없으면 벡터 스토어를 검색하여 문항과 관련된 내용을 찾아 모범답안을 만듭니다. *** 참조 링크 없이, 파일에 적혀 있는 페이지 번호를 있는 그대로 보여주세요. "쪽" 이라는 단어를 사용합니다. 벡터 스토어에서 서술형 문항과 관련된 입력한 파일에서 내용을 찾을 수 없다면 "관련된 내용을 찾을 수 없습니다."라고 보여주세요. ** 반드시 벡터 스토어를 검색하여 서술형 문항과 관련된 내용을 찾는 과정을 거쳐야 합니다. 반드시 벡터 스토어에 있는 파일 내용으로 모범답안을 작성해야 합니다. 반드시 벡터 스토어에 있는 파일 수준으로 모범답안을 작성해야 합니다. 벡터 스토어를 검색했을 때 여러 파일에서 관련된 내용을 찾을 수 있다면 여러 파일의 내용을 종합적으로 활용합니다. ** 어떤 파일에서 어떤 페이지를 보면 되는지 함께 보여주세요. 이때, 반드시 파일 이름과 함께 **그 파일 안에 적혀있는 페이지 숫자**를 보여주어야 합니다. **문단을 나눠서 서술형 평가 문항과 모범답안을 함께 보여줍니다. 서술형 평가 문항과 모범답안을 보여줄 때 **줄을 바꿔서** 구분하여 보기 쉽게 보여줍니다.')
